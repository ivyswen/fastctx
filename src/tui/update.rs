...
pub(crate) struct UpdateMessages {
    ...
    pub(crate) restart_codex: &'static str,
    pub(crate) guidance_refreshed: &'static str,
    pub(crate) guidance_apply_required: &'static str,
    pub(crate) guidance_unchanged: &'static str,
...
macro_rules! update_messages {
    ($available_title:expr, $available_body:expr, $pending_title:expr, $pending_body:expr,
     ...
     $runtime_unchanged:expr, $restart_codex:expr, $guidance_refreshed:expr,
     $guidance_apply_required:expr, $guidance_unchanged:expr,
     $page_title:expr, $auto_check_label:expr, $source_label:expr,
     $auto_check_note:expr, $source_note:expr, $sources_title:expr $(,)?) => {
        UpdateMessages {
            ...
            guidance_apply_required: $guidance_apply_required,
            guidance_unchanged: $guidance_unchanged,
...
const EN: UpdateMessages = update_messages!(
    ...
    "Restart Codex to make the new FastCtx build fully effective in existing sessions.",
    "Managed AGENTS guidance was safely refreshed.",
    "Run fastctx apply to finish connecting Codex.",
    "Managed AGENTS guidance could not be refreshed: {detail}",
    "Updates",
    "Automatic checks",
    "Download source",
...
const ZH_CN: UpdateMessages = update_messages!(
    ...
    "请重启 Codex，让新版 FastCtx 在现有会话中完全生效。",
    "已安全刷新 AGENTS 托管引导。",
    "请运行 fastctx apply 完成 Codex 接入。",
    "无法刷新 AGENTS 托管引导：{detail}",
    "更新",
    "自动检查",
    "下载源",
...
const ZH_TW: UpdateMessages = update_messages!(
    ...
    "請重新啟動 Codex，讓新版 FastCtx 在現有工作階段中完全生效。",
    "已安全重新整理 AGENTS 受管引導。",
    "請執行 fastctx apply 完成 Codex 接入。",
    "無法重新整理 AGENTS 受管引導：{detail}",
    "更新",
    "自動檢查",
    "下載來源",
...
const JA: UpdateMessages = update_messages!(
    ...
    "既存のセッションで新しい FastCtx を完全に反映するには、Codex を再起動してください。",
    "管理対象の AGENTS ガイダンスを安全に更新しました。",
    "fastctx apply を実行して Codex への接続を完了してください。",
    "管理対象の AGENTS ガイダンスを更新できませんでした: {detail}",
    "アップデート",
    "自動確認",
    "ダウンロード元",
...
const KO: UpdateMessages = update_messages!(
    ...
    "기존 세션에 새 FastCtx 빌드를 완전히 적용하려면 Codex를 다시 시작하세요.",
    "관리되는 AGENTS 안내를 안전하게 새로 고쳤습니다.",
    "fastctx apply를 실행해 Codex 연결을 완료하세요.",
    "관리되는 AGENTS 안내를 새로 고칠 수 없습니다: {detail}",
    "업데이트",
    "자동 확인",
    "다운로드 소스",
...
const ES: UpdateMessages = update_messages!(
    ...
    "Reinicia Codex para que la nueva versión de FastCtx se aplique por completo a las sesiones existentes.",
    "La guía AGENTS administrada se actualizó de forma segura.",
    "Ejecuta fastctx apply para completar la conexión con Codex.",
    "No se pudo actualizar la guía AGENTS administrada: {detail}",
    "Actualizaciones",
...
const FR: UpdateMessages = update_messages!(
    ...
    "Redémarrez Codex pour que la nouvelle version de FastCtx soit pleinement active dans les sessions existantes.",
    "Les consignes AGENTS gérées ont été actualisées en toute sécurité.",
    "Exécutez fastctx apply pour terminer la connexion à Codex.",
    "Impossible d’actualiser les consignes AGENTS gérées : {detail}",
    "Mises à jour",
...
const DE: UpdateMessages = update_messages!(
    ...
    "Starten Sie Codex neu, damit der neue FastCtx-Build in bestehenden Sitzungen vollständig wirksam wird.",
    "Die verwalteten AGENTS-Hinweise wurden sicher aktualisiert.",
    "Führen Sie fastctx apply aus, um die Codex-Verbindung abzuschließen.",
    "Die verwalteten AGENTS-Hinweise konnten nicht aktualisiert werden: {detail}",
    "Updates",
...
const PT_BR: UpdateMessages = update_messages!(
    ...
    "Reinicie o Codex para que a nova versão do FastCtx entre totalmente em vigor nas sessões existentes.",
    "A orientação AGENTS gerenciada foi atualizada com segurança.",
    "Execute fastctx apply para concluir a conexão com o Codex.",
    "Não foi possível atualizar a orientação AGENTS gerenciada: {detail}",
    "Atualizações",
...
const RU: UpdateMessages = update_messages!(
    ...
    "Перезапустите Codex, чтобы новая сборка FastCtx полностью заработала в существующих сеансах.",
    "Управляемые инструкции AGENTS безопасно обновлены.",
    "Выполните fastctx apply, чтобы завершить подключение к Codex.",
    "Не удалось обновить управляемые инструкции AGENTS: {detail}",
    "Обновления",
...
const IT: UpdateMessages = update_messages!(
    ...
    "Riavvia Codex affinché la nuova build di FastCtx diventi pienamente attiva nelle sessioni esistenti.",
    "Le indicazioni AGENTS gestite sono state aggiornate in sicurezza.",
    "Esegui fastctx apply per completare il collegamento a Codex.",
    "Impossibile aggiornare le indicazioni AGENTS gestite: {detail}",
    "Aggiornamenti",
...
const TR: UpdateMessages = update_messages!(
    ...
    "Yeni FastCtx derlemesinin mevcut oturumlarda tamamen etkin olması için Codex'i yeniden başlatın.",
    "Yönetilen AGENTS yönlendirmesi güvenle yenilendi.",
    "Codex bağlantısını tamamlamak için fastctx apply çalıştırın.",
    "Yönetilen AGENTS yönlendirmesi yenilenemedi: {detail}",
    "Güncellemeler",
    "Otomatik denetim",
    "İndirme kaynağı",
...
const PL: UpdateMessages = update_messages!(
    ...
    "Uruchom ponownie Codex, aby nowa kompilacja FastCtx w pełni działała w istniejących sesjach.",
    "Zarządzane wskazówki AGENTS zostały bezpiecznie odświeżone.",
    "Uruchom fastctx apply, aby dokończyć połączenie z Codex.",
    "Nie udało się odświeżyć zarządzanych wskazówek AGENTS: {detail}",
    "Aktualizacje",
...
const NL: UpdateMessages = update_messages!(
    ...
    "Start Codex opnieuw zodat de nieuwe FastCtx-build volledig actief wordt in bestaande sessies.",
    "De beheerde AGENTS-instructies zijn veilig vernieuwd.",
    "Voer fastctx apply uit om de verbinding met Codex te voltooien.",
    "De beheerde AGENTS-instructies konden niet worden vernieuwd: {detail}",
    "Updates",
...
const VI: UpdateMessages = update_messages!(
    ...
    "Hãy khởi động lại Codex để bản dựng FastCtx mới có hiệu lực đầy đủ trong các phiên hiện có.",
    "Hướng dẫn AGENTS được quản lý đã được làm mới an toàn.",
    "Chạy fastctx apply để hoàn tất kết nối Codex.",
    "Không thể làm mới hướng dẫn AGENTS được quản lý: {detail}",
    "Bản cập nhật",
    "Tự động kiểm tra",
    "Nguồn tải xuống",
...
const ID: UpdateMessages = update_messages!(
    ...
    "Mulai ulang Codex agar build FastCtx baru berlaku sepenuhnya di sesi yang sudah ada.",
    "Panduan AGENTS terkelola berhasil disegarkan dengan aman.",
    "Jalankan fastctx apply untuk menyelesaikan koneksi Codex.",
    "Panduan AGENTS terkelola tidak dapat disegarkan: {detail}",
    "Pembaruan",
...
const UK: UpdateMessages = update_messages!(
    ...
    "Перезапустіть Codex, щоб нова збірка FastCtx повністю запрацювала в наявних сеансах.",
    "Керовані інструкції AGENTS безпечно оновлено.",
    "Виконайте fastctx apply, щоб завершити підключення до Codex.",
    "Не вдалося оновити керовані інструкції AGENTS: {detail}",
    "Оновлення",
    ...
    fn every_update_locale_is_complete_and_keeps_placeholders() {
        for language in ALL_LANGUAGES {
            ...
            for value in [
                ...
                messages.guidance_refreshed,
                messages.guidance_apply_required,
                messages.guidance_unchanged,
                messages.page_title,
            ...
            assert!(messages.guidance_apply_required.contains("fastctx apply"));
            assert_eq!(messages.guidance_unchanged.matches("{detail}").count(), 1);
        }
    }
}
